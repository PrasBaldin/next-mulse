...
export default function Hero() {
    ...
    // Daftar gambar untuk carousel parallax (gunakan path absolut sesuai folder public)
    const images = ["/img/parallax-1.webp", "/img/parallax-2.webp"];
    const [currentIndex, setCurrentIndex] = useState(0);

    // Preload semua gambar agar tidak ada gambar kosong saat pergantian
    useEffect(() => {
        images.forEach((src) => {
            const img = new Image();
            img.src = src;
        ...
        <>
            <div className="hero-wrap relative">
                <div className="flex justify-center items-center">
                    {/* Container parallax dengan multiple image layer */}
                ...
                <div className="container">
                    <div className="p-3 w-full lg:w-[55vw] absolute top-[35vh] max-lg:left-0 max-lg:right-0 text-gray-100 transition duration-300 ease-in-out">
                        <div className="py-5">
                            ...
                            <p className="md:text-xl font-medium">
                                Solusi konstruksi berkualitas yang memenuhi standar industri terbaik dan didukung oleh
                                tim profesional berpengalaman yang menangani setiap proyek dengan ketelitian serta
                                keahlian. Dengan komitmen terhadap kualitas, kami menciptakan bangunan yang kokoh,
                                fungsional, dan estetis untuk kepuasan maksimal.
                            </p>
                            <div className="pt-5">
                    ...
                    {/* Navigasi Carousel: Tombol bulat */}
                    <div className="absolute top-[95vh] right-5 flex space-x-2">
                        {images.map((_, index) => (
                            <button
                                key={index}
                                onClick={() => handleNavigation(index)}
                                className={`w-3 h-3 rounded-full ${index === currentIndex ? "bg-sky-500" : "bg-gray-300"}`}
                            />
                        ))}
                    </div>
                </div>
