            ...
            <div className="max-w-full md:max-w-[85rem] mx-auto md:absolute top-[100vh]">
                <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-7">
                    ...
                    {benefitRefs.map((ref, index) => {
                        ...
                        const initialTranslate = 1000 * (index + 1);
                        return (
                            <div key={index} ref={ref} className={`transition transform duration-300 ease-in-out p-6 xl:col-span-2 ${benefitData[index].bg} ${benefitData[index].duration} ${benefitData[index].z}`} style={{ transform: `translateX(-${initialTranslate}px)` }}>
