...
const ToggleNightModeMobile = () => {
    ...
    return (
        <button onClick={() => setIsDarkMode(!isDarkMode)} className="btn h-11 flex items-center">
            <div className="flex">
                <div className="flex items-center gap-2">
                    <div className={`toggle-dark-mode-mobile w-11 gap-2 ${isDarkMode ? 'opacity-100 rotate-0' : 'opacity-0 rotate-90'}`} >
                        <IconLightMode />
                    </div>
                    <span className={`text-gray-700 dark:text-gray-300 ${isDarkMode ? 'opacity-100' : 'opacity-0'}`}>Light Mode</span>
                </div>
                <div className="flex items-center gap-2">
                    <div className={`toggle-dark-mode-mobile w-11 gap-2 ${isDarkMode ? 'opacity-0 -rotate-90' : 'opacity-100 rotate-0'}`} >
                        <IconDarkMode />
                    </div>
