...
const Navbar = () => {
    ...
    return (
        <>
                        ...
                        <div className="hidden md:flex space-x-6 items-center">
                            ...
                            >
                                <button className="flex items-center text-gray-700 hover:text-blue-500 transition">
                                    ...
                                    >
                                        {/* Icon panah berbentuk V dengan dua garis */}
                                        <div
                                            className={`w-2.5 h-2.5 border-t-2 border-gray-700 inline-block transform transition-transform duration-300 ${isDropdownOpen ? "rotate-45" : "rotate-0"
                                                }`}
                                        ></div>
                                        <div
                                            className={`w-2.5 h-2.5 border-t-2 border-gray-700 inline-block transform transition-transform duration-300 ${isDropdownOpen ? "-rotate-45" : "rotate-0"
                                ...
                                {/* Dropdown Menu */}
                                <div
                                    className={`absolute left-0 mt-2 w-48 bg-white shadow-lg rounded-lg overflow-hidden transition-all duration-300 ${isDropdownOpen ? "opacity-100 visible scale-100" : "opacity-0 invisible scale-95"
                                        }`}
                                >
                                    <Link
            ...
            <div className={`fixed top-16 left-0 w-full h-full bg-black/50 backdrop-blur-sm transition-opacity duration-300 ${isOpen ? "opacity-100 visible" : "opacity-0 invisible"}`} onClick={() => setIsOpen(false)}
            ></div>
            ...
            <div className={`fixed top-16 right-0 w-64 h-full bg-white shadow-lg transform transition-transform duration-300 ${isOpen ? "translate-x-0" : "translate-x-full"}`}  >
                <div className="p-6 flex flex-col space-y-4">
                    ...
                    <div>
                        <button className="flex items-center text-gray-700 hover:text-blue-500 transition w-full text-left" onClick={() => setIsDropdownOpen(!isDropdownOpen)} >
                            Services
                        </button>
                        {isDropdownOpen && (
                            <div className="ml-4 mt-2 space-y-2 bg-gray-100 p-2 rounded-lg">
                                <Link href="/pengadaan-barang" className="block text-gray-700 hover:text-blue-500 transition" onClick={() => setIsOpen(false)} >
                                    Pengadaan Barang
                                </Link>
                                <Link href="/jasa-konstruksi" className="block text-gray-700 hover:text-blue-500 transition" onClick={() => setIsOpen(false)} >
                                    Jasa Konstruksi
                                </Link>
                            </div>
                        )}
