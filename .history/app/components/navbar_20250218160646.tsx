            ...
            <nav className="bg-white shadow-md fixed w-full z-50">
                <div className="max-w-6xl mx-auto px-4">
                    <div className="flex justify-between items-center py-2">
                        {/* Logo */}
                        <Link href="/" className="text-2xl font-bold text-gray-900 py-3">
                            MyLogo
                        </Link>

                        {/* Menu untuk Desktop */}
                        <div className="hidden md:flex space-x-6 items-center">
                            <Link href="/" className="py-3 text-gray-700 hover:text-blue-500 transition border-b-2 border-transparent hover:border-blue-500">
                                Home
                            </Link>

                            {/* Menu Services dengan Dropdown (Hover di Desktop) */}
                            <div className="relative group" onMouseEnter={() => setIsDropdownOpen(true)} onMouseLeave={() => setIsDropdownOpen(false)} >
                                <button className="flex items-center py-3 text-gray-700 group-hover:text-blue-500 transition border-b-2 border-transparent group-hover:border-blue-500">
                                    Services
                                </button>
                                <div className={`absolute left-0 mt-2 w-56 bg-white border border-gray-200 shadow-xl rounded-lg overflow-visible transition-all duration-300 ${isDropdownOpen ? "opacity-100 visible scale-100" : "opacity-0 invisible scale-95"}`} >
                                    <Link href="/pengadaan-barang" className="block px-4 py-3 text-gray-700 hover:bg-blue-200 transition" >
                                        Pengadaan Barang
                                    </Link>
                                    <Link href="/jasa-konstruksi" className="block px-4 py-3 text-gray-700 hover:bg-blue-200 transition" >
                                        Jasa Konstruksi
                                    </Link>
                                </div>
                            </div>

                            <Link href="/about" className="py-3 text-gray-700 hover:text-blue-500 transition border-b-2 border-transparent hover:border-blue-500">
                                About
                            </Link>
                            <Link href="/contact" className="py-3 text-gray-700 hover:text-blue-500 transition border-b-2 border-transparent hover:border-blue-500">
                                Contact
                            </Link>
                        </div>
