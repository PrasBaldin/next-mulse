        ...
        <>
                    ...
                    <div className="flex justify-between items-center py-4">
                        ...
                        <div className="hidden md:flex space-x-6 items-center">
                            ...
                            <div className="relative group" onMouseEnter={() => setIsDropdownOpen(true)} onMouseLeave={() => setIsDropdownOpen(false)} >
                                ...
                                {/* Dropdown Menu */}
                                <div className={`absolute left-0 mt-2 w-48 bg-white border border-gray-200 shadow-lg rounded-lg overflow-hidden transition-all duration-300 ${isDropdownOpen ? "opacity-100 visible scale-100" : "opacity-0 invisible scale-95"}`} >
                                    <Link href="/pengadaan-barang" className="block px-4 py-2 text-gray-700 hover:bg-blue-100 transition" >
                                        Pengadaan Barang
                                    </Link>
                                    <Link href="/jasa-konstruksi" className="block px-4 py-2 text-gray-700 hover:bg-blue-100 transition" >
                                        Jasa Konstruksi
                                    </Link>
                                </div>
                            </div>
            ...
            <div className={`fixed top-16 right-0 w-64 h-full bg-white shadow-lg transform transition-transform duration-300 ${isOpen ? "translate-x-0" : "translate-x-full"}`}  >
                <div className="p-6 flex flex-col space-y-4">
                    <Link href="/" className="padding-b-1 border-b border-gray-200 text-gray-700 hover:text-blue-500 transition" onClick={() => setIsOpen(false)}>
                        Home
                    </Link>

                    {/* Menu Services dengan Dropdown di Sidebar (Klik untuk Buka) */}
                    <div>
                        <button className="border-b border-gray-200 flex items-center text-gray-700 hover:text-blue-500 transition w-full text-left" onClick={() => setIsDropdownOpen(!isDropdownOpen)} >
                            Services
                        </button>
                        {isDropdownOpen && (
                            <div className="ml-4 mt-2 space-y-2 p-2 rounded-lg">
                                <Link href="/pengadaan-barang" className="border-b border-gray-200 block text-gray-700 hover:text-blue-500 transition" onClick={() => setIsOpen(false)} >
                                    Pengadaan Barang
                                </Link>
                                <Link href="/jasa-konstruksi" className="border-b border-gray-200 block text-gray-700 hover:text-blue-500 transition" onClick={() => setIsOpen(false)} >
                                    Jasa Konstruksi
                                </Link>
                            </div>
                        )}
                    ...
                    <Link href="/about" className="border-b border-gray-200 text-gray-700 hover:text-blue-500 transition" onClick={() => setIsOpen(false)}>
                        About
                    </Link>
                    <Link href="/contact" className="border-b border-gray-200 text-gray-700 hover:text-blue-500 transition" onClick={() => setIsOpen(false)}>
                        Contact
                    </Link>
                </div>
            </div>
