            ...
            <div className="max-w-6xl mx-auto px-4">
                <div className="flex justify-between items-center py-4">
                    ...
                    <button className="md:hidden relative w-8 h-8" onClick={() => setIsOpen(!isOpen)}>
                        <div
                            className={`absolute w-7 h-[3px] bg-gray-800 rounded transition-all duration-300 ${isOpen ? "rotate-45 top-1/2 transform -translate-y-1/2" : "top-2"
                                }`}
                        ></div>
                        <div
                            className={`absolute w-7 h-[3px] bg-gray-800 rounded transition-all duration-300 ${isOpen ? "opacity-0" : "top-1/2 transform -translate-y-1/2"
                                }`}
                        ></div>
                        <div
                            className={`absolute w-7 h-[3px] bg-gray-800 rounded transition-all duration-300 ${isOpen ? "-rotate-45 bottom-1/2 transform translate-y-1/2" : "bottom-2"
                                }`}
                        ></div>
                    </button>
                </div>
